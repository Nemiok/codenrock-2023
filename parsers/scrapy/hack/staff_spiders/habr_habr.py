...
class HabrHabrSpider(scrapy.Spider):
    ...
    def login(self, response, **cb_kwargs):
        if cb_kwargs['start_url'] is None:
            cb_kwargs['start_url'] = 'https://career.habr.com/resumes/habr'
    ...
    def parse_page(self, response, **cb_kwargs):
        ...
        for page in range(2, total_pages + 1):
            ...
            for user in users_data['resumes']['list']:
                ...
                cb_kwargs['age'] = user['age']['title'] if user.get('age') is not None else None
                cb_kwargs['profil_link'] = response.urljoin(user['href'])
                cb_kwargs['avatar'] = response.urljoin(user['avatar']['src2x'])
                cb_kwargs['username'] = user['id']
                cb_kwargs['conversationHref'] = response.urljoin(user['conversationHref'])
                cb_kwargs['skills'] = [i.get('title') for i in user['skills']]
                cb_kwargs['specializations'] = [i.get('title') for i in user['specializations']]
                cb_kwargs['location'] = user['location']['title'] if user.get('location') is not None else None
                cb_kwargs['university'] = user['education']
                cb_kwargs['lastJob'] = user['lastJob']
                yield cb_kwargs
